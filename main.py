...
def train_or_retrain(model_type, split_func):
    ...
    if not SyncOrm.select_model(folder):
        ...
        model = Model(model_type=model_type, dataset_path=data.output_dir, folder=folder)
        model.train()
    ...
    # Проверка необходимости дообучения
        ...
        data = DataSpliting(dataset_path)
        split_func(data)
        model = Model(model_path=model_path, dataset_path=data.output_dir, folder=folder, imgsz=imgsz)
